...
pub fn get_users(username: String, passwordhash: String, state: State<DB>) -> String {
    println!("Processing request username: {}, passwordhash: {}", username, passwordhash);
    let mut db = state.db.lock().unwrap();
    match db.get_table_mut(0) {
        None => {
            format!("{}", "NULL")
        }
        Some(table) => match table.query_by_value(username) {
            None => {
                format!("{}", "NULL")
            }
            Some(user) => {
...
pub fn create_user(user: User, state: State<DB>) -> String {
    println!("Processing request user: {:?}", user);
    let mut db = state.db.lock().unwrap();
    match db.get_table_mut(0) {
        None => {
            format!("{}", "NULL")
...
pub fn create_record(record: Task, _state: State<DB>) -> String {
    print!("Processing request record: {:?}", record);
